...
class EnvWorker(Worker):
    ...
    def env_evaluate_step(
        ...
        eval_success_once = None
        if isinstance(infos, dict) and isinstance(infos.get("episode"), dict):
            success_once = infos["episode"].get("success_once")
            if success_once is not None:
                if torch.is_tensor(success_once):
                    eval_success_once = success_once.bool().cpu().contiguous()
                else:
                    try:
                        eval_success_once = (
                            torch.as_tensor(success_once).bool().cpu().contiguous()
                        )
                    except Exception:
                        eval_success_once = None
        ...
        if chunk_dones.any():
            ...
            if self.cfg.env.eval.env_type == "metaworld":
                ...
                eval_info = self._build_basic_eval_info(env_info, chunk_dones)
    ...
    def _build_metaworld_eval_info(
        ...
        first_success_step = _select(env_info.get("first_success_step", np.array([-1])))
        ...
        for i, env_idx in enumerate(done_indices):
            ...
            first_success_step_val = (
                int(first_success_step[i]) if np.size(first_success_step) > i else -1
            )
            eval_info[int(env_idx)] = {
                ...
                "first_success_step": first_success_step_val,
            }
        return eval_info

    def _build_basic_eval_info(
        self, env_info: dict[str, Any], chunk_dones: torch.Tensor
    ) -> list[dict[str, Any] | None] | None:
        if not torch.is_tensor(chunk_dones):
            return None
        ...
        task_ids = _select(
            env_info.get("task_id", np.full(num_envs, -1, dtype=np.int32))
        ).astype(int)
        trial_ids = _select(
            env_info.get("trial_id", np.full(num_envs, -1, dtype=np.int32))
        ).astype(int)
        reset_state_ids = _select(
            env_info.get("reset_state_id", np.full(num_envs, -1, dtype=np.int32))
        ).astype(int)
        success_arr = _select(success)
        first_success_step = _select(env_info.get("first_success_step", np.array([-1])))
        returns = _select(env_info.get("return", np.array([float("nan")])))
        ...
        for i, env_idx in enumerate(done_indices):
            ...
            success_flag = (
                bool(succ_val)
                if isinstance(succ_val, (bool, np.bool_))
                else bool(succ_val > 0.5)
            )
            eval_info[int(env_idx)] = {
                "episode": int(self._eval_episode_count),
                "task_id": int(task_ids[i]) if np.size(task_ids) > i else -1,
                "trial_id": int(trial_ids[i]) if np.size(trial_ids) > i else -1,
                "reset_state_id": int(reset_state_ids[i])
                if np.size(reset_state_ids) > i
                else -1,
                "success": success_flag,
                ...
                if np.size(first_success_step) > i
                else -1,
            }
        return eval_info
