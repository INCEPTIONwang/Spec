...
class MultiStepRolloutWorker(Worker):
    ...
    async def evaluate(self, input_channel: Channel, output_channel: Channel):
        ...
        eval_forward_calls = [None for _ in range(self.num_pipeline_stages)]
        eval_prev_success_once = [None for _ in range(self.num_pipeline_stages)]
        eval_first_success_forward_calls = [None for _ in range(self.num_pipeline_stages)]
        global_eval_episode_count = 0
        global_first_success_step_sum = 0.0
        global_first_success_step_count = 0
        global_first_success_forward_sum = 0.0
        global_first_success_forward_count = 0
        pbar = tqdm(
        ...
        def _ensure_eval_trackers(stage_id: int, num_envs: int):
            if (
                eval_forward_calls[stage_id] is not None
                and len(eval_forward_calls[stage_id]) == int(num_envs)
            ):
                return
            eval_forward_calls[stage_id] = [0 for _ in range(num_envs)]
            eval_prev_success_once[stage_id] = [False for _ in range(num_envs)]
            eval_first_success_forward_calls[stage_id] = [-1 for _ in range(num_envs)]

        def _update_first_success_forward(
            stage_id: int, success_once: np.ndarray | list[bool], num_envs: int
        ):
            _ensure_eval_trackers(stage_id, num_envs)
            try:
                success_arr = np.asarray(success_once, dtype=np.bool_).reshape(-1)
            except Exception:
                return
            usable = min(int(success_arr.size), int(num_envs))
            for env_idx in range(usable):
                curr = bool(success_arr[env_idx])
                prev = bool(eval_prev_success_once[stage_id][env_idx])
                if curr and (not prev) and int(eval_first_success_forward_calls[stage_id][env_idx]) < 0:
                    eval_first_success_forward_calls[stage_id][env_idx] = int(
                        eval_forward_calls[stage_id][env_idx]
                    )
                eval_prev_success_once[stage_id][env_idx] = curr

        def _clear_eval_trackers_on_done(stage_id: int, env_output: dict[str, Any]):
            if eval_forward_calls[stage_id] is None:
                return
            ...
            for env_idx in np.flatnonzero(done_mask):
                idx = int(env_idx)
                if idx >= len(eval_forward_calls[stage_id]):
                    continue
                eval_forward_calls[stage_id][idx] = 0
                eval_prev_success_once[stage_id][idx] = False
                eval_first_success_forward_calls[stage_id][idx] = -1

        def _infer_num_envs(env_output: dict[str, Any]) -> int:
            ...
            dones = env_output.get("dones")
            if torch.is_tensor(dones):
                return int(dones.shape[0])
            if dones is not None:
                done_arr = np.asarray(dones)
                if done_arr.ndim > 0:
                    return int(done_arr.shape[0])
            return 0
        ...
        def _log_eval_info(stage_id: int, eval_info: list[dict[str, Any] | None]):
            nonlocal global_eval_episode_count
            nonlocal global_first_success_step_sum, global_first_success_step_count
            nonlocal global_first_success_forward_sum, global_first_success_forward_count
            ...
            for env_idx, info in enumerate(eval_info):
                if not info:
                    continue
                first_success_forward_calls = -1
                if (
                    eval_first_success_forward_calls[stage_id] is not None
                    and env_idx < len(eval_first_success_forward_calls[stage_id])
                ):
                    first_success_forward_calls = int(
                        eval_first_success_forward_calls[stage_id][env_idx]
                    )
                info["first_success_forward_calls"] = first_success_forward_calls
                if is_metaworld:
                    eval_line = (
                    ...
                    eval_line = (
                        ...
                        f"first_success_step={info.get('first_success_step', -1)} "
                        f"first_success_forward_calls={info.get('first_success_forward_calls', -1)} "
                        f"return={float(info.get('return', float('nan'))):.4f} "
                        f"episode_len={info.get('episode_len', -1)}"
                    )
                self._append_spec_log(eval_line)
                global_eval_episode_count += 1
                first_success_step_val = int(info.get("first_success_step", -1))
                if first_success_step_val >= 0:
                    global_first_success_step_sum += float(first_success_step_val)
                    global_first_success_step_count += 1
                if int(first_success_forward_calls) >= 0:
                    global_first_success_forward_sum += float(first_success_forward_calls)
                    global_first_success_forward_count += 1
                ...
                if accept_list:
                    avg_accept = _mean_or_nan(accept_list)
                    avg_exec_accept = (
                        _mean_or_nan(accept_exec_list)
                        if accept_exec_list
                        else avg_accept
                    )
                    ...
                    conf_mu_abs_mean = _mean_or_nan(conf_mu_abs_mean_list)
                    conf_var_mean = _mean_or_nan(conf_var_mean_list)
                    conf_var_max = (
            ...
            if items:
                global_line = (
                    f"{'metaworld' if is_metaworld else env_type}_eval_global "
                    f"episodes={global_eval_episode_count} "
                    f"success_episodes={global_first_success_step_count} "
                    f"avg_first_success_step="
                    f"{(global_first_success_step_sum / global_first_success_step_count) if global_first_success_step_count > 0 else float('nan'):.3f} "
                    f"avg_first_success_forward_calls="
                    f"{(global_first_success_forward_sum / global_first_success_forward_count) if global_first_success_forward_count > 0 else float('nan'):.3f}"
                )
                self._append_spec_log(global_line)
                last = items[-1]
                if is_metaworld:
                    pbar.set_postfix(
                        task=last["task"],
        ...
        for _ in pbar:
            ...
            for _ in range(n_chunk_steps):
                for stage_id in range(self.num_pipeline_stages):
                    ...
                    if eval_success_once is not None:
                        try:
                            _update_first_success_forward(stage_id, success_once, num_envs)
                        except Exception:
                            pass
                    eval_info = env_output.get("eval_info")
                    _log_eval_info(stage_id, eval_info)
                    _ensure_action_plans(stage_id, num_envs)
                    _clear_eval_trackers_on_done(stage_id, env_output)
                    ...
                    if need_plan:
                        ...
                        for env_idx in need_plan:
                            eval_forward_calls[stage_id][int(env_idx)] += 1
            ...
            for stage_id in range(self.num_pipeline_stages):
                env_output = await self.recv_env_output(input_channel, mode="eval")
                eval_success_once = env_output.get("eval_success_once")
                num_envs = _infer_num_envs(env_output)
                if eval_success_once is not None:
                    try:
                        if torch.is_tensor(eval_success_once):
                            success_once = eval_success_once.bool().cpu().numpy().reshape(-1)
                        else:
                            success_once = np.asarray(eval_success_once, dtype=np.bool_).reshape(-1)
                    except Exception:
                        success_once = None
                _ensure_eval_trackers(stage_id, num_envs)
                if eval_success_once is not None and num_envs > 0 and success_once is not None:
                    try:
                        _update_first_success_forward(stage_id, success_once, num_envs)
                    except Exception:
                        pass
