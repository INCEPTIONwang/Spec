...
class LiberoEnv(gym.Env):
    def __init__(self, cfg, num_envs, seed_offset, total_num_processes, worker_info):
        ...
        self.enable_history = bool(cfg.get("enable_history", False))
        self.history_window = int(cfg.get("history_window", 2))
        if self.history_window < 2 or self.history_window % 2 != 0:
            raise ValueError(
                f"history_window must be an even integer >= 2, got {self.history_window}"
            )
        self.history_steps_per_camera = self.history_window // 2
        if self.enable_history:
            self._main_image_history = [
                deque(maxlen=self.history_steps_per_camera) for _ in range(self.num_envs)
            ]
            self._wrist_image_history = [
                deque(maxlen=self.history_steps_per_camera) for _ in range(self.num_envs)
            ]
        else:
            self._main_image_history = None
            self._wrist_image_history = None
    ...
    def _record_metrics(self, step_reward, terminations, infos):
        ...
        newly_succeeded = terminations & (self.first_success_step < 0)
        self.first_success_step[newly_succeeded] = self.elapsed_steps[newly_succeeded]
    ...
    def _update_image_history(self, full_image_tensor, wrist_image_tensor, env_idx=None, fill=False):
        if not self.enable_history:
            return
        if env_idx is None:
            env_indices = np.arange(self.num_envs)
        else:
            env_indices = np.asarray(env_idx, dtype=np.int64)

        for env_id in env_indices:
            main_frame = full_image_tensor[int(env_id)].clone()
            wrist_frame = wrist_image_tensor[int(env_id)].clone()

            main_buffer = self._main_image_history[int(env_id)]
            wrist_buffer = self._wrist_image_history[int(env_id)]
            if fill or len(main_buffer) == 0:
                main_buffer.clear()
                wrist_buffer.clear()
                for _ in range(self.history_steps_per_camera):
                    main_buffer.append(main_frame.clone())
                    wrist_buffer.append(wrist_frame.clone())
            else:
                main_buffer.append(main_frame)
                wrist_buffer.append(wrist_frame)

    def _build_history_tensor(self, history_buffers):
        history_tensors = []
        for env_id, buffer in enumerate(history_buffers):
            if len(buffer) == 0:
                raise RuntimeError(
                    f"Image history buffer for env {env_id} is empty; reset should populate it first."
                )
            history_tensors.append(
                torch.stack([frame.clone() for frame in buffer], dim=0)
            )
        return torch.stack(history_tensors, dim=0)

    def _wrap_obs(self, obs_list, env_idx=None, fill_history=False):
        ...
        if self.enable_history:
            obs["main_image_history"] = self._build_history_tensor(self._main_image_history)
            obs["wrist_image_history"] = self._build_history_tensor(self._wrist_image_history)
        return obs
    ...
    def reset(
        ...
        obs = self._wrap_obs(self.current_raw_obs, env_idx=env_idx, fill_history=True)
